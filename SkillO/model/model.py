...
class AttentionLayer(nn.Module):
    def __init__(self,data):
        ...
        self.W = nn.Parameter(torch.Tensor(self.encoderHiddendim, self.decoderHiddendim)) # d*d
...
class ReduceState(nn.Module):
    """
    Encoder is Bi-LSTM,Decoder is single-LSTM
    Simply add two hidden states of Encoder
    """
    def __init__(self):
...
#     @timer('model forward')
    def forward(self, x, x_len, y, len_oovs, batch, num_batches, teacher_forcing):
        ...
        batch_size = x.size()[0]
        decoder_states = encoder_states.view(batch_size,1,-1)
