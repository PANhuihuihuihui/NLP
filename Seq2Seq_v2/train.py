...
#     torch.cuda.empty_cache()
    ...
    with tqdm(total=config.epochs) as epoch_progress:
        for epoch in range(start_epoch, config.epochs):
            ...
            avg_val_loss = evaluate(model, val_dataset, epoch)
...
if __name__ == "__main__":
    ...
    vocab = torch.load(config.vocab_dir)
    print("loading data")
    dataset = SampleDataset(config.data_path)
    val_dataset = SampleDataset(config.val_data_path)
